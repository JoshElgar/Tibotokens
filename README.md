...
The server watches `thsottiaux`, catches up on the previous three days after a restart, and polls every two hours except from 1:00–7:00 a.m. San Francisco time. It keeps the strongest relevant signal from a rolling 24-hour window and uses `openai/gpt-5.6-sol`. Run `npm test` from `server` for its mocked test suite.
